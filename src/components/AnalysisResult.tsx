      ...
      <div className="min-h-screen flex flex-col items-center py-12 px-6">
        ...
        >
          ...
          <div className={`w-24 h-24 rounded-full flex items-center justify-center ${
            ...
            {isSafe ? <Shield className="w-12 h-12" /> : <AlertTriangle className="w-12 h-12" />}
          </div>
        </motion.div>
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm font-mono text-foreground break-all">{result.originalUrl}</p>
              </div>
            </div>
            {result.expandedUrl && (
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-sm font-mono text-foreground break-all">{result.expandedUrl}</p>
                </div>
              </div>
            )}
            <div className="flex items-start gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <p className="text-xs text-muted-foreground">Scanned in {result.scanDuration}ms</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <ThreatBreakdown flags={heuristics} />
        </motion.div>
        ...
        >
          <h3 className="font-display text-sm text-muted-foreground mb-3 tracking-wide">RECOMMENDATIONS</h3>
        ...
        >
          <ExportAnalysis result={result} heuristics={heuristics} />
        </motion.div>

        {/* Action buttons */}
        <motion.div className="flex gap-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}>
          <Button variant="outline" onClick={onClose} className="border-muted-foreground/50">
            Close
          </Button>
          {isSafe && onOpenLink && (
            <Button onClick={onOpenLink} className="bg-safe text-safe-foreground hover:bg-safe/90">
              <ExternalLink className="w-4 h-4 mr-2" />
              Open Link
            </Button>
          )}
          {!isSafe && !isMalicious && onOpenLink && (
            <Button variant="outline" onClick={onOpenLink} className="border-warning text-warning hover:bg-warning/10">
              <ExternalLink className="w-4 h-4 mr-2" />
              Proceed Anyway
            </Button>
          )}
