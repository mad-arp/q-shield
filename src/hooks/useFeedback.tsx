...
export const useFeedback = () => {
  ...
  const triggerHaptic = useCallback(async (type: FeedbackType) => {
    ...
    if (isNativePlatform()) {
      try {
        switch (type) {
          case 'scan': await Haptics.impact({ style: ImpactStyle.Medium }); break;
          case 'safe': await Haptics.notification({ type: NotificationType.Success }); break;
          case 'threat': await Haptics.notification({ type: NotificationType.Error }); break;
          case 'warning': await Haptics.notification({ type: NotificationType.Warning }); break;
          case 'click': await Haptics.impact({ style: ImpactStyle.Light }); break;
        }
      } catch (e) {}
      return;
    }
    ...
    if ('vibrate' in navigator) {
      try { navigator.vibrate(hapticPatterns[type]); } catch (e) {}
    }
  }, []);
  ...
  const feedback = useCallback((type: FeedbackType) => {
    triggerHaptic(type);
    if (type === 'threat') playThreatAlert();
    else if (type === 'safe') playSafeSound();
    else playSound(type);
  }, [triggerHaptic, playSound, playThreatAlert, playSafeSound]);

  return { feedback, triggerHaptic, playSound, playThreatAlert, playSafeSound };
};
